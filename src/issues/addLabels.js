...
module.exports = exports = function(body, issueNumber, repoName, repoOwner) {
  ...
  github.issues.getLabels({
    ...
    github.issues.getIssueLabels({ // retrieve all labels from issue
      owner: repoOwner,
      repo: repoName,
      number: issueNumber
    }).then((issueLabelArray) => {
      issueLabelArray.data.forEach(issueLabel => issueLabels.push(issueLabel.name)); // add all issue label names to issueLabels
